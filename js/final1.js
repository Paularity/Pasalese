$(document).ready(function () {
  ...
  $("#msform").on("submit", async function (e) {
    ...
    if ($("#acount").val() == "") {
      ...
      formData.push({
        name: "file_project",
        value: $(this).find("input[name=file_profile]")[0].files,
      });

      $file_profile = await readFile(
        $(this).find("input[name=file_profile]")[0].files
      );
      var data = getFormData($(this));
      var data = $.extend(data, { "action": "expert_register" });
      var data = $.extend(data, { "file_profile": $file_profile });
  ...
  async function readFile(f) {
    ...
    //     console.log(FR.readAsDataURL(f[0]));
    //   });
    // }
  }
  // $(".next").click(function(){
